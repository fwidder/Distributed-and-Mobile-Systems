...
public class EchoWebServer {
	...
	private int findContentLength(List<String> lines) {
		...
		if (line != null && line.toUpperCase().startsWith("CONTENT-LENGTH")) { // find the following int value
	...
	public void run() {
		...
		boolean postRequest = moreToRead && clientRequestLine.toUpperCase().startsWith("POST");
		while (moreToRead) {
			...
			if (postRequest) { // determine length of POST content
    ...
    public static int PORT = 8080; // some unused port number
	...
	try {
	    serverSocket = new ServerSocket(EchoWebServer.PORT);
	    serverSocket.setSoTimeout(2000); // timeout for accept
	    System.out.println("Server started at " + InetAddress.getLocalHost() + " on port " + EchoWebServer.PORT);
	} catch (IOException e) {
