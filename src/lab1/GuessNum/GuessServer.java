...
public class GuessServer {
	...
	public void run() {
		...
		pw.println("Guess the number between " + min + " and " + (max - 1) + " inclusive");
		int guess = min - 1;
		do {
    ...
    public static int PORT = 7777; // some unused port number
	...
	try {
	    serverSocket = new ServerSocket(GuessServer.PORT);
	    System.out.println("Server started at " + InetAddress.getLocalHost() + " on port " + GuessServer.PORT);
	} catch (IOException e) {
